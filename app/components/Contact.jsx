...
const Contact = ({isDarkMode}) => {
    ...
    const onSubmit = async (event) => {
      ...
      if (data.success) {
        setResult("Submitted Successfully (^_^)");
        event.target.reset();
      } else {
	...
	return (
        ...
        id='contact' className={`w-full px-[12%] py-6 scroll-mt-20 bg-no-repeat bg-center`} 
        style={{ 
            ...
            onSubmit={onSubmit} className='mx-2-2xl mx-auto'>
                ...
                name='message' className={`w-full p-4 outline-none border-[0.5px] border-gray-400 
                     rounded-md mb-6 ${isDarkMode?"border-white/90 bg-[#00001a]":"bg-white "}`} rows='6' placeholder='Enter your message' required></motion.textarea>
                <motion.button
                ...
                type='submit'>Submit now <Image src={assets.right_arrow_white} alt='' className='w-4'></Image></motion.button>
